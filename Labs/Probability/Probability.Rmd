...
[II. Converting DAGs to joint distributions][]

[III. Converting joint distributions to DAGs][]

[IV. Simplifying][]

[V. Interpreting and factoring][]

[VI. Diamond's pigeons][]
...
The probability of an observation $y$ depends on a true ecological state of interest, $z$, and the parameter in a data model, $\theta_d$. The probability of the true state $z$ depends on the parameter in an ecological process model, $\pmb\theta_p$. We know that $\theta_d$ and $\theta_p$ are independent.  Write out a factored expression for the joint distribution, $\Pr\big(y,z,\theta_d,\theta_p\big)$. Drawing a Bayesian network will help.

<div class="hider">
$$\Pr\big(y,z,\theta_d,\theta_p\big) = \Pr\big(y \mid z,\theta_d\big)\Pr\big(z \mid \theta_p\big)\Pr\big(\theta_d\big)\Pr\big(\theta_p\big)$$
...
The holy grail in Bayesian analysis is to discover the *marginal posterior distribution* from unobserved quantities (parameters, latent states, missing data, forecasts) from quantities we are able to observe (data).  It follows that we must understand what marginal distributions are.  The following is an example of a discrete case that also exercises your newly gained familiarity with the laws of probability.

Jared Diamond studied the distribution of fruit pigeons *Ptilinopus rivoli* and *P. solomensis* on 32 islands in the Bismark archipelago northeast of New Guinea (Table 1). Define the event $R$ as an island being occupied by *P. rivoli*, and the event $S$ as an island being occupied by *. solomensis*. The complimentary events are that an island is not occupied by *P. solomensis* $\big(S^c\big)$ and not occupied by *P. rivoli* $\big(R^c\big)$.
...
------------- | -------------
*P. rivoli* present, *P.solomensis* absent | 9
*P. solomensis* present, *P. rivoli* absent | 18
Both present | 2
Both absent | 3
Total | 32
</center>
...
$$ y_i \propto \textrm{Poisson}\big(g\big(\mathbf\theta,x_i\big)\big).$$
</div>
...
10. Simulate 10,000 data points from a normal distribution with mean 0 and standard deviation 1 and another 10,000 data points from a log normal distribution with mean 0 and standard deviation 1.  Display side-by-side histograms scaled to the density.  Overlay the probability density function.  Find the mean, the median, and the variance of in each case (hint: use moment matching for the lognormal case). Check your estimates empirically with the simulated data. Repeat with different means and standard deviations of your choice.
...
log.y <- rlnorm(10000, meanlog = mean, sdlog = sd)
...
11. Existing literature shows that the proportion of habitat patches containing at least one rare plant individual is $\phi=0.04$ with a standard deviation ($\sigma$) of 0.01. 

a. Write out a model predicting the proportion of occupied habitat patches, $y_i$, conditional on $\phi$ and $\sigma$.  
...
\phi&=0.04 \\
\sigma&=0.01\\
\alpha &=\frac{(\phi^2 - \phi^3 - \phi\sigma^2)}{\sigma^2}\\
\beta &=\frac{(\phi - 2\phi^2 + \phi^3 - \sigma^2 + \phi\sigma^2)}{\sigma^2}\\
y_i &\propto \operatorname{beta}(\alpha,\beta)
\end{align}
$$
</div>

- Plot the probability distribution of $\phi$.  
    
<div class="hider">
```{r tidy=TRUE, tidy.opts=list(width.cutoff=100), fig.widght=6, fig.height=5, fig.align='center', echo=key, include=key}
...
# calc prior parameters
b <- shape_from_stats(mu=phi, sigma=sd)
x <- seq(0,.2,.001)
y <- dbeta(x,b[1],b[2])
plot(x,y,type='l', 
     ylab = expression(paste("P(",phi,")")), 
     xlab = expression(phi),
     xlim = c(0,0.1))
...
- If you visited 50 patches, what is the probability that the first 5 would be occupied, conditional on the stated hypothesis ($\phi=0.04$)? Plot the probability of the data.

<div class="hider">
$$
\begin{align}
\Pr(y_i=5|\phi,n=50) &= \operatorname{binomial}(y_i=5|\phi=0.04,n=50)\\
&=\binom{50}{5}0.04^5(1-0.04)^{50-5}\\
\end{align}
$$
```{r tidy=TRUE, tidy.opts=list(width.cutoff=100), fig.widght=6, fig.height=5, fig.align='center', echo=key, include=key}

#probability of 5 occupied
x <- 5
phi <- 0.04
n <- 50
...
#plot the propability of the data
x <- seq(0,10)
y <- dbinom(x=x,p=phi,size=n)
plot(x,y,typ='h',
     ylab=expression(paste("P(y|",phi,")"),),
     xlab=expression(y))

```
</div>

- What is the probability that at least 5 are occupied, conditional on the stated hypothesis ($\phi=0.04$)?  

<div class="hider">
$$
\begin{align}
\Pr(y_i\geq5|\phi,n=50) &= \operatorname{binomial}(y_i\geq5|\phi=0.04,n=50)\\
&=\Sigma_{{y_i}\in(1,2...5)}\binom{50}{5}0.04^{y_i}(1-0.04)^{50-y_i}\\
\end{align}
$$
```{r tidy=TRUE, tidy.opts=list(width.cutoff=100), fig.widght=6, fig.height=5, fig.align='center', echo=key, include=key}

#prob at least 5 occupied
q <- 4
phi <- 0.04
n <- 50
...
plot(x,y,
     ylab=expression(paste("P(",Y<=y,"|",phi,")")),
     xlab=expression(y))

```
</div>

- Simulate data for 75 patches (empty patch = 1, occupied patch = 0).
    
<div class="hider">
$$
\begin{align}
y_i &\propto \operatorname{binomial}(\phi,1)\text{, which is the same as}\\
y_i &\propto \operatorname{Bernoulli}(\phi)
\end{align}
$$
```{r tidy=TRUE, tidy.opts=list(width.cutoff=100), fig.widght=6, fig.height=5, fig.align='center', echo=key, include=key}

#simulate 0-1 data, which is equivalent to Bernoulli since size=1
n <- 75
size <- 1
phi <- 0.04

y.sim <- rbinom(n=n,size=size,prob=phi)
y.sim
```
</div>

12. The Poisson distribution is often used for count data, despite the fact that one must assume that the mean and variance are equal. The negative binomial distribution is a more robust alternative, allowing the variance to differ from the mean. There are two parameterizations for the negative binomial.  The first is more frequently used by ecologists and the second is more often implemented in coding environments (i.e. JAGS). 

$$
\begin{align}
[z|\lambda,\kappa] = \frac{\Gamma(z + \kappa)}{\Gamma(\kappa)z!}\Big(\frac{\kappa}{\kappa+\lambda}\Big)^\kappa\Big(\frac{\lambda}{\kappa+\lambda}\Big)^z \\
\end{align}
$$

where $z$ is a discrete random variable, $\lambda$ is the mean of the distribution and $\kappa$ is the *dispersion parameter*.

$$
\begin{align}
[z|\kappa,\phi] &= \frac{\Gamma(z+\kappa)}{\Gamma(\kappa)z!}\phi^\kappa(1-\phi)^z
\end{align}
$$

where $z$ is the random variable representing the number of failures that occur in a sequence of Bernoulli trials before a target number successes, $\kappa$, are obtained. The parameter $\phi$ is the probability of success on a given trial. 

- Simulate 10000 observations from a negative binomial distribution with mean 100 and variance 400 using the first parameterization that has a mean and a dispersion parameter (remember to moment match).

<div class="hider">
```{r tidy=TRUE, tidy.opts=list(width.cutoff=100), fig.widght=6, fig.height=5, fig.align='center', echo=key, include=key}
## r (kappa) is dispersion called "size" in R
n <- 10000
lambda <- 100
sigma <- sqrt(400)
r <- lambda^2/(sigma^2-lambda)
z <- rnbinom(n,mu=lambda,size=r)
mean(z)
sd(z)
```
</div>

- Do the same simulation using the probability using the other parameterization. Note that $\phi=\kappa/(\lambda+\kappa)$

<div class="hider">
```{r tidy=TRUE, tidy.opts=list(width.cutoff=100), fig.widght=6, fig.height=5, fig.align='center', echo=key, include=key}
## Parameterized for JAGS using probabilty parameter
n <- 10000
lambda <- 100
sigma <- sqrt(400)
r <- lambda^2/(sigma^2-lambda)
z2 <- rnbinom(n,prob=r/(r+lambda),size=r)
mean(z2)
sd(z2)
...
lambda <- 100
r <- lambda^2/(sigma^2-lambda)
sigma <- sqrt(lambda + lambda^2/r)
z <- rnbinom(n, mu=lambda, size=r)
```
</div>

- Plot side-by-side histograms of the simulated data (Note: R's hist() function isn't well suited for discrete data. Hint: see count() in the plyr package.
...
z <- rnbinom(n,mu=lambda,size=r) #first parameterization
z2 <- rnbinom(n,prob=r/(r+lambda),size=r) #second parameterization
