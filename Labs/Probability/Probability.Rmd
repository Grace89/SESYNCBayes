...
11. Simulate 10,000 data points from a normal distribution with mean 0 and standard deviation 1 and another 10,000 data points from a log normal distribution with mean 0 and standard deviation 1.  Display side-by-side histograms scaled to the density.  Overlay the probability density function.  Find the mean, the median, and the variance of in each case (hint: use moment matching for the lognormal case). Check your estimates empirically with the simulated data. Repeat with different means and standard deviations of your choice.
...
12. Existing literature shows that the proportion of habitat patches containing at least one rare plant individual is $\phi = 0.04$ with a standard deviation $\big(\sigma\big)$ of 0.01. Write out a model predicting the proportion of occupied habitat patches, $y_{i}$, conditional on $\phi$ and $\sigma$.  

<div class="hider">
$$
\begin{align}
\alpha &=\cfrac{\big(\phi^2 - \phi^3 - \phi\sigma^2\big)}{\sigma^2}\\
\beta &=\cfrac{\big(\phi - 2\phi^2 + \phi^3 - \sigma^2 + \phi\sigma^2\big)}{\sigma^2}\\
y_i &\sim \textrm{beta}\big(\alpha,\beta\big)
\end{align}
$$
</div>
...
b <- shapeit(mu = phi, sigma = sd)
x <- seq(0, .2, .001)
y <- dbeta(x, b[1], b[2])

plot(x, y,type = 'l', ylab = expression(paste("Pr(",phi,")")), xlab = expression(phi), 
  xlim = c(0, 0.1), lwd = 2)
```

<br>

14. If you visited 50 patches, what is the probability that 5 would be occupied, conditional on the stated hypothesis $\big(\phi=0.04\big)$? 
...
15. Plot the probability of the data for y = 1...10 occupied patches out of 50 patches visited conditional on the stated hypothesis $\big(\phi=0.04\big)$.
...
16. What is the probability that at least 5 are occupied, conditional on the stated hypothesis ($\phi=0.04$)?  

<div class="hider">
\begin{eqnarray}
\Pr\big(y \geq5 \mid \phi, n=50\big) & = & \textrm{binomial}\big(y \geq5 \mid \phi=0.04,n=50\big)\\[1em]
&= &\sum_{y_{i}\in(5,6,\ldots,50)}\binom{50}{5}\big(0.04\big)^{\,y_{i}}\big(1-0.04\big)^{50-y_{i}}\\[1em]
\end{eqnarray}
</div>
...
17. Plot the cumulative probability that at least y = 1...10 patches are occupied, conditional on the stated hypothesis ($\phi=0.04$)?  
