...
It is important to practice using conjugate distributions when possible. This requires you to select a prior that is “conjugate to the likelihood” and write out the posterior distribution in closed form. For each scenario below:
...
You are conducting a political opinion poll and poll a small number of voters at random, $n=10$. You ask each voter whether they will vote "yes" in an upcoming referendum on funding for local climate adaptation. Seven voters say they intend to vote "no"" and 3 report intending to vote "yes". Little is known about how people might vote.  Assuming a vague prior on the probability that a person votes "yes", answer questions A-D. In addition:

* Estimate the probability that the referendum passes as $\textrm{Pr}\big(\phi \geq .5\big)$. 

<div class="hider">
\begin{eqnarray}
\big[\phi \mid y \big] & \propto & 
...
prior <- function(x) dnorm(x, muPrior, varPrior)
post <- function(x) dnorm(x, muPost, varPost)

plot(prior, 0, 60, main = "Normal prior (dash) and normal posterior (red)", xlab = expression(mu), ylab = "Density", lty = 2, ylim = c(0,0.1), lwd = 2)
